...
train_data_god=np.load('embedded_train_data.npy')
train_labels_god=np.load('train_labels.npy')
...
print(device)
for num_model in range(16,80):
	model = nn.Sequential(nn.Linear(512, 18),
						nn.ReLU(),
	...
	#model=torch.load('bestfullyconnected_512_40_20')
	...
	indy=np.random.randint(0, train_labels_god.shape[0], size=int(train_labels_god.shape[0]/4))
	train_labels=train_labels_god[indy]
	train_data=train_data_god[indy]
	...
	def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
		...
		for epoch in range(num_epochs):
			...
			for phase in ['train', 'test']:
				...
				epoch_loss = running_loss / 10000/batch_size
				
				epoch_acc = running_corrects.float() / 10000/batch_size
		...
		return model
	model = train_model(model, criterion, optimizer_ft, plateau_scheduler,num_epochs=100)
	torch.save(model,'bestfullyconnected_512_18_ensemble'+str(num_model))